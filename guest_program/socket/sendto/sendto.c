...
//	s2e_make_symbolic(&flags, sizeof(int), "send_flags");
	...
	int nbytes2 = sizeof(buf);//?
	int flags2 = MSG_DONTROUTE;//?
	s2e_make_symbolic(&nbytes2, sizeof(int), "send_len");
	s2e_make_symbolic(&flags2, sizeof(int), "send_flags");
	s2e_make_symbolic(&buf,sizeof(buf), "sendto_buf");
	s2e_make_symbolic(&clen, sizeof(int), "sendto_addr_len");
	sendto(s, buf, nbytes2, flags2, &addr, clen);//sendto
