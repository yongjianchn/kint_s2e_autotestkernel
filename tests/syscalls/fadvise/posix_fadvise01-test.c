...
int main(int ac, char **av)
{
	...
	for (lc = 0; TEST_LOOPING(lc); lc++) {
		...
		for (i = 0; i < defined_advise_total; i++) {
			...
			s2e_enable_forking();
			s2e_make_symbolic(&tst1, sizeof(tst1), "tst1");
			s2e_make_symbolic(&tst2, sizeof(tst2), "tst2");
			s2e_make_symbolic(&defined_advise[i], sizeof(defined_advise[i]), "defined_advise");
			
			//TEST(posix_fadvise(fd, 0, 0, defined_advise[i]));
			TEST(posix_fadvise(fd, tst1, tst2, defined_advise[i]));
			s2e_disable_forking();
