...
int main(int ac, char **av)
{
	...
	for (lc = 0; TEST_LOOPING(lc); lc++) {
		...
		for (i = 0; i < TST_TOTAL; i++) {
			...
			s2e_enable_forking();
			//s2e_make_symbolic(TC[i].fd, sizeof(int), "TC[i].fd");
			s2e_make_symbolic(&TC[i].request, sizeof(int), "TC[i].request");
			s2e_make_symbolic(TC[i].s_tio, sizeof(struct termio), "TC[i].s_tio");
