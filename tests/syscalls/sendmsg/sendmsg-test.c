...
int main(int argc, char *argv[])
{
	...
	for (lc = 0; TEST_LOOPING(lc); ++lc) {
		tst_count = 0;
		for (testno = 3; testno < TST_TOTAL; ++testno) {
			tdat[testno].setup();
			...
			s2e_enable_forking();
			s2e_make_symbolic(&(msgdat.msg_iov),sizeof(msgdat.msg_iov), "msgdat.msg_iov");
			s2e_make_symbolic(&(msgdat.msg_iovlen),sizeof(msgdat.msg_iovlen), "msgdat.msg_iovlen");
			//TEST(sendmsg(s, tdat[testno].msg, tdat[testno].flags));
			sendmsg(s, &msgdat, tdat[testno].flags);
			s2e_disable_forking();
			s2e_kill_state(0," killstate in test!");
